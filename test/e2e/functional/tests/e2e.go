...
var _ = Describe("metallb", func() {
	...
	Context("MetalLB Core Components Log Level Feature", func() {
		It("Verify logLevel default and debug behavior in speaker and controller pods", func() {
			...
			speakerPods, err := testclient.Client.Pods(OperatorNameSpace).List(context.Background(), metav1.ListOptions{
				LabelSelector: "component=speaker",
			})
			Expect(err).ToNot(HaveOccurred(), "Failed to list speaker pods")
			Expect(len(speakerPods.Items)).Should(BeNumerically(">", 0), "Speaker Pods List should not be empty")
